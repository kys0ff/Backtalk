...
@Composable
fun ImageCompressionSelectionDialog(
    selected: ImageCompressionLevel,
    onDismiss: () -> Unit,
    onSelected: (ImageCompressionLevel) -> Unit,
) {
    ...
    AlertDialog(
        ...
        title = { Text(text = stringResource(R.string.settings_image_compression)) },
        text = {
            Column(
                ...
                ImageCompressionLevel.entries.forEach { level ->
                    val isSelected = level == tempSelected
                    Surface(
                        selected = isSelected,
                        onClick = { tempSelected = level },
